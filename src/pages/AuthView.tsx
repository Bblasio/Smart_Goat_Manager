        ...
        <section className="p-6 sm:p-10 lg:col-span-3">
          <div className="mx-auto max-w-md space-y-6">
            ...
            {mode === 'signup' && <form onSubmit={handleSignup} className="space-y-3">
              <div><h2 className="text-2xl font-bold">Create your farm account</h2></div>
              ...
              <div className="relative">{iconInput(<Lock className="h-4 w-4" />, { id: 'input-signup-password', type: showPassword ? 'text' : 'password', value: password, onChange: e => setPassword(e.target.value), placeholder: 'Password', required: true })}<button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-3.5 text-stone-400">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div>
